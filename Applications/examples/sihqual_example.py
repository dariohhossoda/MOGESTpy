...
def simple_channel_example():
    ...
    model = SIHQUAL(dx=dx, dt=dt, xf=xf, tf=tf, alpha=0.6, dispersion_coef=5.0)
    ...
    q_values = np.array(
        [base_flow + flow_amplitude * np.sin(2 * np.pi * t / 86400) for t in time_array]
    ).reshape(1, -1)
    ...
    lateral_c_data = {"time": time_array, "values": lateral_c_values}

    model.set_lateral_inflows(
        lateral_Q_segments, lateral_Q_data, lateral_c_segments, lateral_c_data
    )
...
def plot_results(results):
    ...
    for section in sections:
        axes[0].plot(results.index, results[(section, "Q")], label=f"Section {section}")
    ...
    # Plot water depth
    for section in sections:
        axes[1].plot(results.index, results[(section, "y")], label=f"Section {section}")
    ...
    # Plot concentration
    for section in sections:
        axes[2].plot(results.index, results[(section, "c")], label=f"Section {section}")
