...
def simple_channel_example():
    ...
    model.set_simple_upstream_flow(
        flow_values=[15.0, 20.0, 15.0],
        # Flow varying over time
        times=[0, 43200, 86400],
    )

    model.set_simple_downstream_level(level_values=1.5)  # Constant downstream level
...
def trapezoidal_channel_example():
    ...
    model.set_uniform_initial_conditions(depth=2.5, velocity=0.6, concentration=0.0)

    # Set reaction parameters
    model.set_uniform_reaction_parameters(
        decay_coef=0.00005, source_coef=0.0  # Decay coefficient  # No source
    )
    ...
    boundary_data = {"c": {"time": time_array, "values": np.array([conc_values])}}
