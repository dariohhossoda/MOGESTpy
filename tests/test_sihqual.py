...
class TestSIHQUAL:
    ...
    def test_auto_step_adjustment(self):
        ...
        q_values = np.array([[200.0, 200.0, 200.0]])  # m³/s (very high flow)
        y_values = np.array([[2.0, 2.0, 2.0]])  # m
        ...
        # Force courant_max to a high value to trigger adjustment
        model.courant_max = 2.0
    ...
    def test_non_uniform_grid(self):
        ...
        # From 3m at upstream to 1m at downstream
        y_initial = np.linspace(3.0, 1.0, model.dim)

        # Create non-uniform initial velocity
        # From 0.3 m/s at upstream to 0.7 m/s at downstream
        v_initial = np.linspace(0.3, 0.7, model.dim)
